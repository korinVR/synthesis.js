...
class Track {
  ...
  update (currentTick, seeking) {
    ...
    while (this.nextEventTick < currentTick) {
      // send MIDI message
      const statusByte = this.readByte()
      const statusUpper4bits = statusByte >> 4

      // meta event
      if (statusByte === 0xff) {
        const metaEventType = this.readByte()
        const length = this.readByte()

        if (metaEventType === 0x51) {
          if (length === 3) {
            const quarterMicroseconds = this.readByte() << 16 | this.readByte() << 8 | this.readByte()
      ...
      if (statusByte === 0xf0) {
        const systemExclusive = [statusByte]
      ...
      switch (statusUpper4bits) {
        ...
        case 0xe:
        {
          const dataByte1 = this.readByte()
          const dataByte2 = this.readByte()
...
export default class SMFPlayer {
  ...
  onInterval () {
    ...
    const tickTime = this.quarterTime / this.timebase
