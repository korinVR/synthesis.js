  ...
  render (buffer, length, sampleRate) {
    if (this.state !== STATE_OFF) {
      for (let i = 0; i < length; i++) {
        const amplitude = this.synthesizer.modulationWheel * this.vibratoAmplitude

        const vibratoPeriod = sampleRate / this.vibratoFrequency
        this.vibratoPhase += 1 / vibratoPeriod
        const vibratoOffset = this.vibratoOscillator.getSample(this.vibratoPhase) * amplitude

        const frequency = this.note2frequency(this.note + this.synthesizer.pitchBend + vibratoOffset)
